            ...
            for line_rep in ${replaseKV}; do
                ...
                if [ "x${os}" = x"windows" ]; then
                    ...
                    if [[ "${arch}" == *"${line_rep_k}" ]]; then
                        cp -f ${frp_dir_name}/${frp_action}.exe $FRP_OPT_DIR/${frp_action}_${os}_${arch%%${line_rep_k}}${line_rep_v}.exe
                    fi
                    if [[ "${arch}" == *"${line_rep_v}" ]]; then
                        cp -f ${frp_dir_name}/${frp_action}.exe $FRP_OPT_DIR/${frp_action}_${os}_${arch%%${line_rep_v}}${line_rep_k}.exe
                    fi
                else
                    tar -zxf ${frp_dir_name}.tar.gz
                    cp -f ${frp_dir_name}/${frp_action} $FRP_OPT_DIR/${frp_action}_${os}_${arch}
                    if [[ "${arch}" == *"${line_rep_k}" ]]; then
                        cp -f ${frp_dir_name}/${frp_action} $FRP_OPT_DIR/${frp_action}_${os}_${arch%%${line_rep_k}}${line_rep_v}
                    fi
                    if [[ "${arch}" == *"${line_rep_v}" ]]; then
                        cp -f ${frp_dir_name}/${frp_action} $FRP_OPT_DIR/${frp_action}_${os}_${arch%%${line_rep_v}}${line_rep_k}
                    fi
                fi
            done
        done
